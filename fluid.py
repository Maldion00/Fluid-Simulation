...
if __name__ == "__main__":
    ...
    fluids_len = int((len(digit_list) - 2) / 21)
    ...
    for i in range(2, len(digit_list)): 
        if digit_list[i][0].lstrip('-').isnumeric():
            if i == 2 or (i % 23) - 2 == 0:
                den_positions_x1.append(int(digit_list[i][0]))
            elif i == 3 or (i % 23) - 3 == 0:
                den_positions_y1.append(int(digit_list[i][0]))
            elif i == 6 or (i % 23) - 6 == 0:
                den_positions_x2.append(int(digit_list[i][0]))
            elif i == 7 or (i % 23) - 7 == 0:
                den_positions_y2.append(int(digit_list[i][0]))
            elif i == 10 or (i % 23) - 10 == 0:
                den_increases.append(int(digit_list[i][0]))
            elif i == 13 or (i % 23) - 13 == 0:
                velo_positions_x.append(int(digit_list[i][0]))
            elif i == 14 or (i % 23) - 14 == 0:
                velo_positions_y.append(int(digit_list[i][0]))
            elif i == 17 or (i % 23) - 17 == 0:
                velo_increases_x.append(int(digit_list[i][0]))
            elif i == 18 or (i % 23) - 18 == 0:
                velo_increases_y.append(int(digit_list[i][0]))
        if i == 21 or (i % 23) - 21 == 0:
            rotations.append(digit_list[i][0])
        if i == len(digit_list) - 2:
            color_map = digit_list[i][0]
    ...
    try:
        ...
        def update_im(i):
            ...
            for a in range(fluids_len):
                # We add new density creators in here
                inst.density[den_positions_x1[a]:den_positions_x2[a], den_positions_y1[a]:den_positions_y2[a]] += den_increases[a]  # add density into a 3*3 square
                # We add velocity vector values in here

                if rotations[a] == 'ROTATE_R':
                    inst.velo[velo_positions_x[a], velo_positions_y[a]] = [velo_increases_x[a], velo_increases_y[a]]

                elif rotations[a] == 'ROTATE_L':
                    inst.velo[velo_positions_x[a], velo_positions_y[a]] = [velo_increases_x[a], velo_increases_y[a]]
                elif rotations[a] == 'NULL':
                    inst.velo[velo_positions_x[a], velo_positions_y[a]] = [velo_increases_x[a], velo_increases_y[a]]
        ...
        fig = plt.figure()
        # plot density
        im = plt.imshow(inst.density, cmap=color_map, vmax=100, interpolation='bilinear')
        # plot vector field
